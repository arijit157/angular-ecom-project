...
export class SellerService {
  ...
  sellerLoggedError=new EventEmitter<boolean>(false);

  constructor(private http: HttpClient, private router: Router) { }

  sellerSignupService(data: sellerSignup) {
  ...
  sellerLoginService(data: sellerLogin) {
    return this.http.get(`http://localhost:3000/seller?email=${data.email}&password=${data.password}`, { observe: "response" }).subscribe((result: any) => {
      ...
      if (result.body && result.body.length > 0) {
        ...
        this.sellerLoggedError.emit(true);
      }
    });
  }
}
